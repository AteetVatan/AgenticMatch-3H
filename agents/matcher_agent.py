...
class MatcherAgent:
    def __init__(self):
        ...
        self.partner_metadata = JsonHelper.get_json_data("partner_metadata.json")
        """
        0 → Tokyo Beauty Lab  
        1 → Nordic Face Studio  
        2 → Mediterraneo Glow  
        3 → Parisian Porcelaine  
        4 → Ayurvedic Radiance
        """
        self.id_to_partner = JsonHelper.get_json_data("id_to_partner.json")

    def match(self, image_pil):
        vector = self.embedding_agent.get_vector(image_pil)
